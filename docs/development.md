...
│   └── audio.js        # Web Audio API を用いた効果音・BGM管理
└── docs/
    ├── spec.md
    └── development.md
```
...
<head>
  ...
  <style>
    body { margin: 0; background: #222; display: flex; justify-content: center; align-items: center; height: 100vh; font-family: 'Press Start 2P', monospace; }
    canvas { display: block; border: 4px solid #555; background: #000; box-shadow: 0 0 20px rgba(0,0,0,0.5); }
  </style>
</head>
<body>
...
15,000 個のブロックを `Uint8Array` で管理する。値の割り当ては以下の通り：
- `0`: 破壊済
- `1〜5`: 各色のブロック（値に応じたスコアを持つ）
...
| `←` / `→` または `A` / `D` | パドル左右移動 |
| マウスの水平移動 | パドル左右移動 |
| `Space` / クリック | ゲーム開始 / ボール発射（Catchアイテム取得後） |
| `ESC` / 画面外クリック | ゲーム一時停止（PAUSE） |
| `P` | 一時停止（デバッグ用） |
...
効果音（SE）の実装には、外部の音声ファイル（mp3/wav）を読み込む代わりに、ブラウザ標準の **Web Audio API (OscillatorNode)** を用いたプロシージャルな音源生成を推奨します。
- **メリット1**: 15,000ブロックの破壊時など、短い間隔で大量に音が鳴る場合でも遅延が少なく軽量です。
- **メリット2**: 連続破壊時に音の周波数（ピッチ）を動的に計算して上げるなどのインタラクティブな表現が簡単に実装できます。
- **注意点**: ブラウザの自動再生ポリシーにより、ユーザーが画面をクリック（またはキー操作）するまで音声コンテキストを再生できないため、初回入力時に `AudioContext` を初期化（または resume）する処理が必要です。
...
未破壊の全ブロックは初期化時に別の `Canvas` (`OffscreenCanvas`) にキャッシュ描画し、毎フレームの `render` ではそのキャッシュ画像を1回の `drawImage` で転送する。ブロック破壊時は `blocks.js` 内でキャッシュ画像から該当矩形を `clearRect` して抜くのが最も高速。
